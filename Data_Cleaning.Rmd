...
```{r}
# Example reference points (you can add as many as you like)
reference_points <- data.frame(
  lon_center = c(174.80, 174.51, 174.68, 175.03, 174.56),
  lat_center = c(-36.94, -36.90, -36.70, -36.92, -36.34)
)

# Total number of random points to generate
n_points <- 11926

# Buffer size in degrees (approx. 0.01 ≈ 1 km)
lon_buffer <- 0.025  # ~2 km east/west
lat_buffer <- 0.025  # ~2 km north/south

# ---- Function to generate random points around one reference ----
generate_points <- function(lon_center, lat_center, n_sub) {
  ...
  data.frame(
    Latitude = runif(n_sub, min = lat_min, max = lat_max),
    Longitude = runif(n_sub, min = lon_min, max = lon_max)
  )
}

# ---- Distribute total points roughly evenly among reference points ----
n_refs <- nrow(reference_points)
points_per_ref <- rep(floor(n_points / n_refs), n_refs)
# Add remainder points (if not divisible evenly)
points_per_ref[1:(n_points %% n_refs)] <- points_per_ref[1:(n_points %% n_refs)] + 1

# ---- Generate all points ----
random_points_list <- mapply(
  generate_points,
  lon_center = reference_points$lon_center,
  lat_center = reference_points$lat_center,
  n_sub = points_per_ref,
  SIMPLIFY = FALSE
)

non_landslides_incomplete <- do.call(rbind, random_points_list)
...
non_landslides_complete <- do.call(rbind, random_points_list)
...
leaflet(combined_data_incomplete) %>%
  addTiles() %>%  # base map
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
